When I initially learned about the Collatz conjecture, the video claimed that each number had a unique number of steps in the sequence. My first thought was that the number of upward bumps of 3x+1 could make an interesting encryption key if we changed that to ax+b, where a, x, and b are natural numbers and b must be an odd number.
